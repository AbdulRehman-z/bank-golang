...
type (
	...
	CreateAccountRequest struct {
		...
		Currency string `validate:"required,oneof=USD EUR CAD"` // Required field, one of USD, EUR, CAD
	}
	...
	ListAccountsRequest struct {
		PageID   int32 `validate:"required,gt=1"` // Required field, min 1
		PageSize int32 `validate:"required,lt=5"` // Required field, min 5
	}
